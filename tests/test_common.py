...
class CommonTests(unittest.TestCase):
    ...
    def test_mirror_source_accepts_explicit_per_abi_matrix(self):
        # Apps whose mirrors publish no universal package declare one matrix
        # entry per ABI instead of taking the implicit contract.
        entries = source_architecture_entries({
            "type": "mirrors",
            "archs": [
                {"name": "arm64"},
                {"name": "arm32"},
        ...
        self.assertEqual([e["name"] for e in entries], ["arm64", "arm32"])
        src = {"type": "mirrors",
               "archs": [{"name": "arm64"}, {"name": "arm32"}]}
        self.assertEqual(resolve_arch_entry(src, "arm32")["name"], "arm32")
        with self.assertRaises(RuntimeError):
            resolve_arch_entry(src, None)

    def test_mirror_source_rejects_universal_mixed_with_concrete(self):
    ...
    def test_validate_universal_refuses_unknown_abi_set(self):
        # A missing native-code line means the ABI set is unknown, not empty.
        # The old code skipped the check silently; that let an ARM32-only or
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaisesRegex(RuntimeError, "no native-code"):
                validate_package(
                    path, "apk", expected_version="1.2.3",
                    aapt_path=aapt, expected_arch="universal",
                )
