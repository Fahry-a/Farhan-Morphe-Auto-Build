...
# NOTE: this module must stay importable without third-party dependencies.
# The build workflow's `setup` job imports it on a bare runner (no pip
# install happens there) to generate the app/arch matrix, so `apkd` is only
# ever imported lazily inside the functions that run where it is installed.


def _required_abis(arch):
    """Canonical ABIs for `arch`, via apkd when available.

    apkd.providers.base is the single definition of the universal contract;
    this wrapper exists only so matrix generation works without it installed.
    """
    try:
        from apkd.providers.base import required_abis
    except ImportError:
        value = str(arch or "").strip().lower().replace("_", "-")
        if value in ("universal", "noarch"):
            return {"arm64-v8a", "armeabi-v7a"}
        return {value} if value else set()
    return required_abis(arch)


def source_architecture_entries(source):
    """Return architecture entries for a source configuration.

    Mirror-backed sources default to one implicit contract: ``universal``.
    They do not need a hand-written ABI list in every app JSON file. A
    ``source.archs`` block is accepted in two shapes:

    * exactly ``[{"name": "universal"}]`` — the same implicit contract,
      spelled out;
    * an explicit list of concrete ABI names (``arm64``, ``arm32``, ...)
      for apps whose mirrors publish no universal package at all. Each entry
      becomes its own matrix job downloading and validating that ABI.

    Mixing ``universal`` with concrete ABIs is rejected: a job is either the
    both-ABI contract or one ABI, never both. Whether a concrete name is a
    real ABI is checked at download time (where apkd is installed), not here,
    so matrix generation keeps working on a bare runner.
    """
    source = source or {}
    ...
    if source.get("type") == "mirrors":
        ...
        if names == [MIRROR_DEFAULT_ARCH]:
            return [{"name": MIRROR_DEFAULT_ARCH}]
        if not names or MIRROR_DEFAULT_ARCH in names or "noarch" in names:
            raise ValueError(
                "source.type=mirrors takes either the implicit universal "
                "contract (no source.archs) or an explicit list of concrete "
                "ABI names; universal must not be mixed with concrete ABIs"
            )
        return entries
    return entries
...
def resolve_arch_entry(src, arch):
    ...
    Mirror sources default to the single universal contract, or to their
    explicit per-ABI entries when the app's mirrors publish no universal
    package. Other source types retain their explicit architecture entries
    and legacy top-level fields.
    """
    # Always normalised through source_architecture_entries: a config spelling
    # an entry as a bare string ("archs": ["arm64"]) used to pass matrix
    # generation and then crash the downloader with AttributeError on
    # entry.get.
...
def _declares_native_code(badging: str) -> bool:
    """Whether aapt emitted a native-code line at all.

    An empty ``native-code:`` value is a real answer (no native libraries). A
    missing line means the ABI set is unknown, and the two must not be conflated.
    """
    return re.search(r"^native-code:", badging, re.MULTILINE) is not None
...
def _validate_apk_architecture(path, arch, aapt_path=None):
    ...
    required = _required_abis(arch)
    if not required:
        return detected
    if not _declares_native_code(badging):
        # aapt emitted no native-code line at all. That is different from
        # "declares no native code": it means the ABI set could not be
        # determined, and skipping the check here would let an ARM32-only or
        # x86_64-only asset through as a universal build.
        raise RuntimeError(
            f"{path} declares no native-code line, so arch={arch} cannot be "
            f"verified; aapt must report native-code for a package that ships "
            f"native libraries"
        )
...
def _validate_bundle_architecture(path, arch, aapt_path=None):
    ...
    with tempfile.TemporaryDirectory() as td:
        with zipfile.ZipFile(path) as archive:
            ...
            for index, name in enumerate(apk_names):
                ...
                try:
                    badging = _apk_badging(extracted, aapt_path)
                except RuntimeError as exc:
                    inspection_errors.append(f"{name}: {exc}")
                    continue
                declared_any = declared_any or _declares_native_code(badging)
                detected.update(_native_codes(badging))
    ...
    if not declared_any:
        raise RuntimeError(
            f"{path} declares no native-code in any bundle entry, so "
            f"arch={arch} cannot be verified"
        )
    if detected and not required.issubset(detected):
        missing = ", ".join(sorted(required - detected))
        found = ", ".join(sorted(detected)) or "none"
