...
def download_and_validate(url, output_path, file_type, expected_version, token=None,
    ...
    ``expected_arch`` and ``expected_package`` are forwarded deliberately: a
    GitHub-sourced release is still a package that gets patched and published,
    so its ABI set and identity are part of the contract, not an optional extra.
    """
...
def main():
    ...
    if args.config:
        ...
        file_type = "apk"
    expected_version = args.apk_version or tag.removeprefix(prefix)
    try:
        entries = download_and_validate(
            ...
            expected_version,
            token,
            expected_arch=entry.get("name"),
            expected_package=cfg.get("package"),
        )
    except RuntimeError as e:
