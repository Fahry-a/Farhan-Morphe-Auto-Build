...
try:
    ...
    from .mirror_download import SUPPORTED_MIRRORS, download_from_mirror
except ImportError:  # pragma: no cover - direct ``python tools/...`` use
    from common import source_architecture_entries, validate_package
    from mirror_download import SUPPORTED_MIRRORS, download_from_mirror

# apkd owns the provider list; see mirror_download for why this is not a local copy.
SUPPORTED_MIRRORS = frozenset(SUPPORTED_MIRRORS)
...
def probe_target(
    ...
    *,
    ...
    timeout: float = 30.0,
    downloader: Callable[..., Any] = download_from_mirror,
) -> ProbeResult:
